...
## 💎 Business Context 
Danny founded Fresh Segments, a digital marketing agency that aggregates client customer data to analyze online ad click behavior and interest trends. See the original case study statement [here](https://8weeksqlchallenge.com/case-study-8/).
...
> 💬 **Note**: *‌Converted interest_id to INTEGER to simplify future JOINs with interest_map.id; not required by the case, but verified to be safe for all questions.*
...
> 💬 **Note**: *Use INNER JOIN to keep only metrics with a valid interest in interest_map, ensuring a consistent analysis dataset and excluding orphan IDs.*
...
> 💬 **Note**: *month_year stores the month bucket (first day of month), so it can precede created_at without being invalid.‌* 
